...
def index():
    ...
    for stock in stocks:
        ...
        total_position = current_price * stock["shares"]
        stock["total"] = total_position
        total_stocks_value += total_position
...
def history():
    ...
    history_db = db.execute("SELECT * FROM transactions WHERE user_id = ?", session["user_id"])
